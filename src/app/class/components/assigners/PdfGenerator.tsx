...
// Font registration – using a type assertion to bypass unsafe-call errors
(Font as unknown as { register: (config: unknown) => void }).register({
  family: "Roboto",
  src: "https://cdnjs.cloudflare.com/ajax/libs/ink/3.1.10/fonts/Roboto/roboto-light-webfont.ttf",
});

// Emoji registration – same technique as above
(
  Font as unknown as { registerEmojiSource: (config: unknown) => void }
).registerEmojiSource({
  format: "png",
        ...
        <View style={styles.table}>
          ...
          {rows.map((row, rowIndex) => (
            <View key={rowIndex} style={styles.tableRow}>
              ...
              {groups.flatMap((group) => {
                const itemMap = groupItemsMap[group];
...
const PDFActions = ({ data }: { data: AssignedData }) => {
  ...
  const generatePdfBlob = async () => {
    // Assert that the pdf instance has a toBlob method.
    const pdfInstance = pdf(<AssignerPDF data={data} />) as {
      toBlob: () => Promise<Blob>;
    };

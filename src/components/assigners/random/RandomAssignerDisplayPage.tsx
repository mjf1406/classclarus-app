...
import { useRandomAssignerRunById } from "@/hooks/assigners/random/useRandomAssigners";
import {
...
type RandomAssignerDisplayPageProps = {
  runId: Id<"randomAssignerRuns">;
};

export function RandomAssignerDisplayPage({ runId }: RandomAssignerDisplayPageProps) {
  const { t } = useTranslation("assigners");
  const { data: run, isPending, isError, refetch } = useRandomAssignerRunById(runId);
  ...
  useLayoutEffect(() => {
    ...
    const updateScale = () => {
      ...
      // Measure unscaled size (transform does not affect scrollWidth/Height).
      const next = Math.min(
        1,
  ...
  if (isError) {
    return (
      <div className="flex h-dvh items-center justify-center p-6">
        <ErrorState
          title={t("randomDisplayLoadFailed")}
          description={t("randomDisplayLoadFailedDescription")}
  ...
  if (!run || !matrix) {
    return (
      <div className="flex h-dvh items-center justify-center p-6">
        <ErrorState
          title={t("randomDisplayNotFound")}
          description={t("randomDisplayNotFoundDescription")}
        />
      </div>
    );
  }
      ...
      <header className="shrink-0 border-b px-3 py-2 sm:px-4">
        <div className="flex flex-wrap items-baseline justify-between gap-x-3 gap-y-0.5">
          ...
          <p className="text-xs text-muted-foreground sm:text-sm">
            {t("randomDisplayMeta", {
              scope: formatRandomAssignerScope(run.scope, t),
              replicates: formatRandomAssignerReplicates(run.replicates, t),

...
function breadcrumbTarget(pathname: string, classId: string): BreadcrumbTarget {
  ...
  if (pathname === `${base}/classroom-screen` || pathname.startsWith(`${base}/classroom-screen/`)) {
    return { kind: "classroomScreen" };
  }
  if (pathname === `${base}/timers` || pathname.startsWith(`${base}/timers/`)) {
    return { kind: "timers" };
  }
  if (pathname === `${base}/rotations` || pathname.startsWith(`${base}/rotations/`)) {
    return { kind: "rotations" };
  }
  if (pathname === `${base}/audio` || pathname.startsWith(`${base}/audio/`)) {
    return { kind: "audio" };
  }
    ...
    target.kind === "attendance"
      ...
      : target.kind === "calendar" || target.kind === "calendarEventDetail"
        ...
        : target.kind === "timetable"
          ? tTimetable("nav")
          : target.kind === "classroomScreen"
            ? tClassroomScreen("nav")
            : target.kind === "timers"
              ? tClassroomScreen("timersTitle")
              : target.kind === "rotations"
                ? tClassroomScreen("rotationsTitle")
                : target.kind === "audio"
                  ? tClassroomScreen("audioTitle")

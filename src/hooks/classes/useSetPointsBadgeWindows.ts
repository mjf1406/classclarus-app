...
type SetPointsBadgeWindowsArgs = {
  classId: Id<"classes">;
...
export function useSetPointsBadgeWindows() {
  ...
  const mutationFn = useConvexMutation(api.classes.setPointsBadgeWindows);
  const listKey = classesListQueryKey();

  return useOptimisticMutation({
    mutationFn: (args: SetPointsBadgeWindowsArgs) => mutationFn(args),
    queryKeys: (args) => [listKey, classDetailQueryKey(args.classId)],
    applyOptimisticUpdate: (queryClient, args) => {
      const detailKey = classDetailQueryKey(args.classId);
      const now = Date.now();
      const patch = {
        warningWindowAmount: args.warningWindowAmount,
    ...
    onError: (error) => {
      toast.add({
        title: messageFromError(error, t("pointsBadgeWindowsSaveFailed"), tCommon("rateLimited")),
        type: "error",
      });
    },
  });

...
type SetStudentLanguageArgs = {
  classId: Id<"classes">;
  studentLanguage: AppLanguage;
};

export function useSetStudentLanguage() {
  const { t } = useTranslation("classes");
  const { t: tCommon } = useTranslation("common");
  const mutationFn = useConvexMutation(api.classes.setStudentLanguage);
  const listKey = classesListQueryKey();

  return useOptimisticMutation({
    mutationFn: (args: SetStudentLanguageArgs) => mutationFn(args),
    queryKeys: (args) => [listKey, classDetailQueryKey(args.classId)],
    applyOptimisticUpdate: (queryClient, args) => {
      const detailKey = classDetailQueryKey(args.classId);
      const now = Date.now();
      const patch = { studentLanguage: args.studentLanguage, updatedAt: now };
    ...
    onError: (error) => {
      toast.add({
        title: messageFromError(error, t("studentLanguageSaveFailed"), tCommon("rateLimited")),
        type: "error",
      });
    },
  });

...
export function useSetTimezone() {
  ...
  const mutationFn = useConvexMutation(api.classes.setTimezone);
  const listKey = classesListQueryKey();

  return useOptimisticMutation({
    mutationFn: (args: SetTimezoneArgs) => mutationFn(args),
    queryKeys: (args) => [listKey, classDetailQueryKey(args.classId)],
    applyOptimisticUpdate: (queryClient, args) => {
      const detailKey = classDetailQueryKey(args.classId);
      const now = Date.now();
      const patch = { timezone: args.timezone, updatedAt: now };
    ...
    onError: (error) => {
      toast.add({
        title: messageFromError(error, t("timezoneSaveFailed"), tCommon("rateLimited")),
        type: "error",
      });
    },
  });

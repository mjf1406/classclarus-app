...
type SetGroupImageArgs = {
  classId: Id<"classes">;
  groupId: Id<"groups">;
  fileId: Id<"files">;
};

export function useSetGroupImage() {
  const { t } = useTranslation("classes");
  const { t: tCommon } = useTranslation("common");
  const mutationFn = useConvexMutation(api.groups.setGroupImage);

  return useOptimisticMutation({
    mutationFn: (args: SetGroupImageArgs) =>
      mutationFn({
        ...
        return {
          ...
          groups: old.groups.map((group) =>
            group._id === args.groupId
              ? { ...group, imageFileId: args.fileId, updatedAt: now }
              : group,
          ),
        };
      });

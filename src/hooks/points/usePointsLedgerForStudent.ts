...
type LedgerPage = FunctionReturnType<typeof api.points.ledgerForStudent>;

export function pointsLedgerForStudentQueryKey(
  classId: Id<"classes">,
  studentUserId: Id<"users"> | null,
) {
  return ["points", "ledgerForStudent", classId, studentUserId] as const;
}

/** gcTime: GC_TIME.realtime — same as personal ledger; revision tip keeps mounted data fresh. */
export function usePointsLedgerForStudent(
  classId: Id<"classes">,
  studentUserId: Id<"users"> | null,
) {
  ...
  const revisionQuery = useQuery({
    ...convexQuery(
      api.points.ledgerRevisionForStudent,
  ...
  const query = useInfiniteQuery({
    queryKey: pointsLedgerForStudentQueryKey(classId, studentUserId),
    ...
    queryFn: async ({ pageParam }) => {
      ...
      return await convex.query(api.points.ledgerForStudent, {
        classId,
        studentUserId,
        limit: PAGE_SIZE,

...
import { classes as classesTable, 
    ...
    users as usersTable } from "@/server/db/schema";
import { sql, eq } from "drizzle-orm";
import { type Course, type Teacher, type Student } from "@/server/db/types";
...
async function databaseClassToCourseMap(data: DataButt): Promise<Course | undefined> {
    ...
    const classData: Course = {
        ...
        complete: data?.class[0]?.classes.complete ?? { s1: false, s2: false },
        teachers: teachers,
        students: students,
    };
    return classData;
